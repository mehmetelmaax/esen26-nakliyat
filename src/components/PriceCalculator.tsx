...
export default function PriceCalculator() {
  ...
  const getWhatsAppLink = () => {
    if (!estimate) return '';
    const text = `Merhaba, Esen 26 Nakliyat web sitesindeki Fiyat Hesaplayıcı ile tahmini bütçe çıkardım:
- Ev Boyutu: ${rooms} Daire
- Mesafe Grubu: ${distanceType === 'sehirici' ? 'Tepebaşı / Odunpazarı Şehiriçi' : distanceType === 'ilceler' ? 'İlçe Sınırları' : `Şehirlerarası (${distanceKm} Km)`}
...
Bu tahmini hesaplama üzerinden detayları netleştirmek ve net teklif almak istiyorum.`;
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-gray-light/60 shadow-premium overflow-hidden text-charcoal">
      {/* Top Banner */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-off-white border border-gray-light rounded-xl px-4 py-2.5 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-orange/30 focus:border-orange transition-all"
                >
                  <option value="sehirici">Eskişehir Şehiriçi (Tepebaşı/Odunpazarı)</option>
                  <option value="ilceler">Eskişehir İlçeler Arası (Sivrihisar/Çifteler vb.)</option>
                  <option value="sehirlerarasi">Şehirlerarası (Eskişehir Dışı 81 İl)</option>
                </select>
              </div>
            </div>

            {/* Kilometer Slider for Intercity */}
            {distanceType === 'sehirlerarasi' && (
              <div className="bg-off-white p-4 rounded-xl border border-gray-light/60 space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-navy uppercase block">Şehirlerarası Mesafe (Km)</label>
                  <span className="text-sm font-bold text-orange-text">{distanceKm} Km</span>
                </div>
                <input
                  type="range"
                  min="50"
                ...
                <span className="text-[10px] text-gray-400 block mt-1 leading-tight font-medium">Şehirlerarası mesafelerde Km başına +30 TL ek yol ücreti hesaplanır.</span>
              </div>
            )}
