...
export default function RelatedLinks({ currentSlug, type, title }: RelatedLinksProps) {
  ...
  const getServiceAnchor = (name: string, index: number, prefix: string = '') => {
    const templates = [
      ...
      `Eskişehir ${prefix ? prefix + ' ' : ''}${name} çözümleri`,
  ...
  if (type === 'hizmet') {
    ...
    // 3 neighbors + 6 services + /eskisehir-nakliyat-fiyatlari
    ...
    if (neighborsList.length < 3) {
      neighborsList = DISTRICTS.filter(d => d.slug !== currentSlug && true).slice(0, 3);
    }
    ...
    links.push({
      ...
      text: `${currentDistrict ? currentDistrict.name : 'Eskişehir'} ev taşıma fiyatları listesi`,
    });
  } 
