...
`;
  ...
  const routesList = [
    ...
    ...ROUTES.map(r => ({
      ...
      name: `Eskişehir - ${r.city} Nakliyat`,
  ...
  for (const item of routesList) {
    ...
    if (fs.existsSync(fullFilePath)) {
      ...
      pageText = `Esen 26 Nakliyat ${item.name} hizmet detayları. Sabit fiyat garantisiyle ${FACTS.insurer} sigortalı ve marangozlu evden eve nakliye çözümleri. Lütfen ${SITE.url}${item.route} adresini ziyaret edin.`;
    }
