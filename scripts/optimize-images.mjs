...
async function run() {
  ...
  // 1. Optimize logo.png
  const logoPath = path.join(imgDir, 'esen-26-logo.png');
  if (fs.existsSync(logoPath)) {
    ...
    // logo.webp
    const logoWebpPath = path.join(imgDir, 'esen-26-logo.webp');
    await sharp(tempLogoPath)
      .resize(400)
      .webp({ quality: 80 })
      .toFile(logoWebpPath);
  ...
  for (const name of photos) {
    ...
    for (const w of widths) {
      // webp (quality 60 for 1920, 75 for others)
      const webpQ = w === 1920 ? 60 : 75;
      ...
      const jpegQ = w === 1920 ? 45 : w === 1280 ? 65 : 75;
